...
const EmployeeExpedient = () => {
    ...
    const getStatusBadge = (status) => {
        switch (status) {
            case 'VERIFIED':
                return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium bg-green-50 text-green-800 border border-green-200"><CheckCircleIcon className="w-3.5 h-3.5" /> Verificado</span>;
            case 'PENDING':
                return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium bg-amber-50 text-amber-800 border border-amber-200"><ClockIcon className="w-3.5 h-3.5" /> En Revisión</span>;
            case 'REJECTED':
                return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium bg-red-50 text-red-800 border border-red-200"><XCircleIcon className="w-3.5 h-3.5" /> Rechazado</span>;
            default:
                return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-600 border border-gray-200"><ExclamationTriangleIcon className="w-3.5 h-3.5" /> Faltante</span>;
        }
    };
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="pb-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                    <p className="text-[11px] font-medium text-gray-400 uppercase tracking-wider mb-0.5">Recursos Humanos · Expediente Digital</p>
                    <h1 className="text-xl font-semibold text-gray-900">Expediente de Colaborador</h1>
            ...
            <div className="bg-white p-4 rounded border border-gray-200 flex flex-col md:flex-row gap-5 items-center justify-between">
                <div className="space-y-1.5 w-full md:w-2/3">
                    <div className="flex justify-between items-center text-xs font-medium text-gray-700">
                        ...
                        <span className="font-mono font-semibold text-gray-900" style={{ fontVariantNumeric: 'tabular-nums lining-nums' }}>{completionPercentage}%</span>
                    </div>
