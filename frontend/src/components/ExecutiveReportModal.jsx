import { FiPrinter, FiShield, FiDollarSign } from 'react-icons/fi';
import Modal from './common/Modal';

/**
 * Modal de Generación e Impresión de Informe Ejecutivo para Directivos / Junta
 * Estilo minimalista y claro integrado con la plataforma con Portal Universal
 */
export default function ExecutiveReportModal({ isOpen, onClose, data }) {
    ...
    return (
        <Modal
            isOpen={isOpen}
            onClose={onClose}
            title="Informe Ejecutivo Organizacional y Financiero"
            size="xl"
            footer={
                <div className="flex items-center justify-between w-full">
                    <button
                        onClick={handlePrint}
                        className="px-3.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs font-medium inline-flex items-center gap-1.5 cursor-pointer shadow-xs transition-colors"
                    >
                        <FiPrinter size={13} /> Imprimir / PDF
                    </button>
                    <button
                        onClick={onClose}
                        className="px-3.5 py-1.5 border border-gray-300 hover:border-gray-400 bg-white hover:bg-gray-50 text-gray-700 rounded text-xs font-medium transition-colors cursor-pointer"
                    >
                        Cerrar
                    </button>
                </div>
        ...
        >
            ...
            <div className="space-y-6 text-slate-800 print:p-0 print:space-y-4 max-h-[70vh] overflow-y-auto pr-1">
                {/* Document Header */}
                <div className="flex justify-between items-start border-b border-slate-200 pb-4">
                    <div>
                        <h1 className="text-xl font-extrabold text-slate-900 tracking-tight">INFORME ESTRATÉGICO EMPRESARIAL</h1>
                ...
                <div className="grid grid-cols-3 gap-4">
                    <div className="p-4 bg-slate-50/80 border border-slate-200 rounded-lg">
                        ...
                        <p className="text-3xl font-extrabold text-blue-600 mt-1">{healthScore} <span className="text-sm font-semibold text-slate-400">/ 100</span></p>
                ...
                <div className="space-y-3">
                    <h3 className="text-sm font-bold text-slate-900 border-b border-slate-200 pb-2 flex items-center gap-2">
                        <FiDollarSign className="text-blue-600" /> Diagnóstico Financiero de Riesgo y Oportunidades
                    </h3>
                ...
                <div className="space-y-3">
                    <h3 className="text-sm font-bold text-slate-900 border-b border-slate-200 pb-2 flex items-center gap-2">
                        <FiShield className="text-blue-600" /> Dictamen Estratégico & Plan de Acción para la Alta Dirección
                    </h3>
                    <div className="space-y-2">
                        <div className="p-3.5 bg-slate-50/80 border-l-4 border-blue-600 text-xs text-slate-700 space-y-1 rounded-r-lg">
                        ...
                        <div className="p-3.5 bg-slate-50/80 border-l-4 border-emerald-500 text-xs text-slate-700 space-y-1 rounded-r-lg">
